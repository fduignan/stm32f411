...
const fptr Vectors[] __attribute__((section(".vectors"))) ={
	...
	USART1_Handler, 	/* 37: USART1 global interrupt */
...
void initClockHSI100Mhz_out_of_spec()
{
	
	// Odd finding here:  With this setup the system clock should be 96MHz.  Based 
	// on the behaviour of the systick interrupt handler however it would seem that the 
	// system clock is EXACTLY 100MHz.
	// HSI was routed through MCO_1 and measured to be 16MHz
	// The value of 48 for the N divisor is below the stated minimum of 50
...
// result in a system with an invalid clock signal - result: a stuck system
        ...
        // SysClock is taken from output R of the PLL.  
		 ...
		 * The M divisor is 2 at least - choose this which means PLL input frequency is 8MHz
		 * This means that the N divisor should be 384/8 = 48
		 */
		...
		RCC->PLLCFGR |= (48 << 6); // Set N Bits
		RCC->PLLCFGR &= ~(0x3f); // Clear M Bits
		RCC->PLLCFGR |= 2; // Set M bits
